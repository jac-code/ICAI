...
%aviobj = avifile('2.avi','fps',25);
aviobj = VideoWriter('4mp4', 'MPEG-4'); %Aquí es donde se va a escribir
open(aviobj);
%motion Vectors
...
N=k-1; %Número de frames

Nframes=floor(N/10)*10; % Redondeo a la potencia de 10 cercana


global I1d P4d P7d I10d B2d B3d B5d B6d B8d B9d
%framedata=aviread('news.avi',1);
%framedata=VideoReader('news.avi');
I1=mov(1).cdata;
%I1=frame2im(frame1);
...
for i=1:10:Nframes
    ...
    if i~=1
        j=i-round(i/10);
        [B2,B3,P4,B5,B6,P7,B8,B9,I10]=GOP_gris(j, mov);
    elseif i==1
        j=i;
        [B2,B3,P4,B5,B6,P7,B8,B9,I10]=GOP_gris(j, mov);
    end
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if i==1
                figure
                imshow(uint8(I1d))
                writeVideo(aviobj, uint8(I1d));
                %aviobj = addframe(aviobj,getframe);
                %clf;
                
                %figure
                imshow(uint8(B2d))
                writeVideo(aviobj, uint8(B2d));
                %aviobj = addframe(aviobj,getframe);
                %clf;
                
                %figure
                imshow(uint8(B3d))
                writeVideo(aviobj, uint8(B3d));
                %aviobj = addframe(aviobj,getframe);
                %clf;
                
                %figure
                imshow(uint8(P4d))
                writeVideo(aviobj, uint8(P4d));
                %aviobj = addframe(aviobj,getframe);
                %clf;
                
                %figure
                imshow(uint8(B5d))
                writeVideo(aviobj, uint8(B5d));
                %aviobj = addframe(aviobj,getframe);
                %clf;
                
                %figure
                imshow(uint8(B6d))
                writeVideo(aviobj, uint8(B6d));
                %aviobj = addframe(aviobj,getframe);
                %clf;
                
                %figure
                imshow(uint8(P7d))
                writeVideo(aviobj, uint8(P7d));
                %aviobj = addframe(aviobj,getframe);
                %clf;
                
                %figure
                imshow(uint8(B8d))
                writeVideo(aviobj, uint8(B8d));
                %aviobj = addframe(aviobj,getframe);
                %clf;
                
                %figure
                imshow(uint8(B9d))
                writeVideo(aviobj, uint8(B9d));
                %aviobj = addframe(aviobj,getframe);
                %clf;
                
                %figure
                imshow(uint8(I10d))
                writeVideo(aviobj, uint8(I10d));
                %aviobj = addframe(aviobj,getframe); 
                %clf;
            else
